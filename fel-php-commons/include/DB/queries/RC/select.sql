SELECT
	[D].[comprobante_serie] + '-' + cast([D].[comprobante_numero] as nvarchar) AS [resumen_serie_numero],
	...
	CONVERT(varchar(10),[B].[resumen_fecha_emision],120) AS [resumen_fecha_emision],
    CONVERT(varchar(10),[B].[resumen_fecha_referencia],120) AS [resumen_fecha_referencia]
FROM
	[dbo].[t_documento] as [D]
INNER JOIN
	[dbo].[t_resumen] as [B] ON
