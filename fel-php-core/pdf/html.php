...
function print_20($name, $documento, $hash, $firma, $sunat){
...
$response = $conn->fetchAssoc("SELECT hash, firma, mensaje_sunat FROM t_documento where identificador = ?", array($name));

header('Content-Type: text/html; charset=utf-8');
print_20($name, $document, $response['hash'], $response['firma'], $response['mensaje_sunat']);